...
// Returns true (and logs why) if any guest thread is currently unsafe to
// serialize: blocked on something other than a semaphore/mutex/event-flag
// wait, or -- defensively -- one of those but not actually parked where
// looks_like_parked_import_call() expects.
bool has_unsafe_thread(KernelState &kernel, MemState &mem) {
    const std::lock_guard<std::mutex> lock(kernel.mutex);
    for (auto &[id, thread] : kernel.threads) {
        if (thread->status != ThreadStatus::wait)
            continue;
        if (!looks_like_parked_import_call(*thread->cpu, mem)) {
            LOG_WARN("Savestate: thread {} ({}) is waiting but not parked on a recognized import-stub call, cannot save right now.", id, thread->name);
            return true;
        }
        if (!is_waiting_in(kernel.semaphores, thread) && !is_waiting_in(kernel.mutexes, thread) && !is_waiting_in(kernel.eventflags, thread)) {
            LOG_WARN("Savestate: thread {} ({}) is waiting on something other than a semaphore/mutex/event flag, cannot save right now.", id, thread->name);
            return true;
        }
    }
    return false;
...
SaveStateResult save_state(EmuEnvState &emuenv, const fs::path &path) {
    ...
    if (has_unsafe_thread(kernel, mem))
